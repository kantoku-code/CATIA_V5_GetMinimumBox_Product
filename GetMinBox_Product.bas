...
'指定したﾌﾟﾛﾀﾞｸﾄを元にMinimumBoxを作成

Option Explicit

Private Const MINBODYNAME = "MinimumBox" 'MinimumBoxName
Private Const DMYLNG = 1000000# 'ﾀﾞﾐｰ面距離
Private Enum MINMAX '測定値配列ｲﾝﾃﾞｯｸｽ用
    MinX = 0
    MaxX = 1
    MinY = 2
    MaxY = 3
...
Private Function getMaxSize_Bodies( _
    ...
    '測定方向用ﾍﾞｸﾄﾙ　ｲﾝﾃﾞｯｸｽはEnum MinMax
    Dim vec As Variant
    vec = Array( _
        Array(-1#, 0#, 0#), _
        Array(1#, 0#, 0#), _
...
Private Sub initConstraint( _
    ...
    If dist < 0.001 Then Exit Sub 'IsMissing(Dist)ﾀﾞﾒ?
